...
for i in range(3):                                      #设置一个循环，目的是循环读取并滤波处理所有三种图片
    ...
    # 高斯滤波
    result = cv2.GaussianBlur(REPLICATE, (3, 3), 0)     #3即为核的大小，在高斯滤波中设置为3

    # 显示图像
    cv2.imshow("source img", img)                       #显示原始图像
    cv2.imshow("gaussianBlur img(3*3)", result)         #显示高斯滤波处理后的图像
     #这里设置了判断语句，以分别对三种不同的原图进行分别保存,根据每种图片的噪声类型，分别命名
    if i == 0:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Gaussian_gaussian.png"),result)
    if i == 1:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Gaussian_origin.png"),result)
    else:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Gaussian_pepper.png"),result)
