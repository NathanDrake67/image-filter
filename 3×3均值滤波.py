...
#print(Image_name_list[::])                              #可检验读取图片的名称及数量
...
for i in range(3):                                       #设置一个循环，目的是循环读取并滤波处理所有三种图片
    ...
    # 均值滤波Mean filtering
    result = cv2.blur(REPLICATE, (3, 3)) #3即为核的大小，在均值滤波中默认为3
    # 显示图像
    cv2.imshow("source img", img)                        #显示原始图像
    cv2.imshow("averageBlur img(3*3)", result)           #显示均值滤波处理后的图像
     #这里设置了判断语句，以分别对三种不同的原图进行分别保存,根据每种图片的噪声类型，分别命名
    if i == 0:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Moving average_gaussian.png"),result)
    if i == 1:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Moving average_origin.png"),result)
    else:
        cv2.imwrite(os.path.join(r"output_1/","3x3_Moving average_pepper.png"),result)
