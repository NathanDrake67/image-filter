# -*- coding: utf-8 -*-
"""
Created on Mon Apr  4 22:08:19 2022
...
for i in range(3):                                 #设置一个循环，目的是循环读取并滤波处理所有三种图片
    ...
    # REPLICATE：通过复制边缘像素补全 ，选定图片上下左右各需要复制边缘像素的宽度范围，这里为滤波器长度的一半，即3
    REPLICATE = cv2.copyMakeBorder(source, 3, 3, 3, 3, cv2.BORDER_REPLICATE)
    # 高斯滤波
    result = cv2.GaussianBlur(REPLICATE, (7,7), 0) #（7.7）即为核的大小，此处设置为7

    # 显示图像
    cv2.imshow("source img", img)                  #显示原始图像
    cv2.imshow("gaussianBlur img(7*7)", result)    #显示高斯滤波处理后的图像
     #这里设置了判断语句，以分别对三种不同的原图进行分别保存，根据每种图片的噪声类型，分别命名
    if i == 0:
        cv2.imwrite(os.path.join(r"output_2/","7x7_Gaussian_gaussian.png"),result)
    if i == 1:
        cv2.imwrite(os.path.join(r"output_2/","7x7_Gaussian_origin.png"),result)
    else:
        cv2.imwrite(os.path.join(r"output_2/","7x7_Gaussian_pepper.png"),result)
